...
namespace apsi::util {
    ...
    void Stopwatch::add_event(const string &name)
    {
        ...
        max_event_name_length_ = std::max(static_cast<int>(name.length()), max_event_name_length_);
    }
    ...
    {
        uint64_t duration =
            static_cast<uint64_t>(chrono::duration_cast<chrono::milliseconds>(end - start).count());
        ...
        if (timespan_evt == timespan_events_.end()) {
            ...
            max_timespan_event_name_length_ =
                std::max(static_cast<int>(name.length()), max_timespan_event_name_length_);
        } else {
            // Update existing
            ...
            timespan_evt->second.min = std::min(timespan_evt->second.min, duration);

            timespan_evt->second.max = std::max(timespan_evt->second.max, duration);
        }
    }
    ...
    StopwatchScope::StopwatchScope(Stopwatch &stopwatch, string event_name)
        : stopwatch_(stopwatch), event_name_(std::move(event_name)),
