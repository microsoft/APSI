...
namespace apsi::util {
    namespace {
        uint64_t read_u64_little_endian(const array<unsigned char, 8> &bytes)
        {
            ...
            val |= static_cast<uint64_t>(bytes[1]) << 8U;
            val |= static_cast<uint64_t>(bytes[2]) << 16U;
            val |= static_cast<uint64_t>(bytes[3]) << 24U;
            val |= static_cast<uint64_t>(bytes[4]) << 32U;
            val |= static_cast<uint64_t>(bytes[5]) << 40U;
            val |= static_cast<uint64_t>(bytes[6]) << 48U;
            val |= static_cast<uint64_t>(bytes[7]) << 56U;
        ...
        array<unsigned char, 8> write_u64_little_endian(uint64_t num)
        {
            ...
            bytes[3] = static_cast<unsigned char>((num & 0x00000000FF000000ULL) >> 24U);
            bytes[4] = static_cast<unsigned char>((num & 0x000000FF00000000ULL) >> 32U);
            bytes[5] = static_cast<unsigned char>((num & 0x0000FF0000000000ULL) >> 40U);
            bytes[6] = static_cast<unsigned char>((num & 0x00FF000000000000ULL) >> 48U);
            bytes[7] = static_cast<unsigned char>((num & 0xFF00000000000000ULL) >> 56U);
            ...
            gsl::span<unsigned char> dest)
        {
            ...
            uint32_t low_offset = bit_offset & 7U;

            // The number of full bytes that should be written to dest
            uint32_t full_byte_count = bit_count >> 3U;

            // The index of the first src word which contains our bits
            uint32_t word_begin = bit_offset >> 3U;

            uint32_t rem_bits = bit_count - (full_byte_count * 8U);
#ifdef APSI_DEBUG
            if (bit_offset + bit_count > src.size() * 8U) {
                throw invalid_argument("invalid split_length, or index out of range");
            }
            if (bit_count > dest.size() * 8U) {
            ...
            if (rem_bits) {
                ...
                if (one_word_src) {
                    ...
                    high = static_cast<unsigned char>(high & ~static_cast<uint32_t>(mask));
                    ...
                    unsigned char low = static_cast<unsigned char>(
                        (static_cast<uint32_t>(src[word_begin]) >> low_offset) & low_mask);
                    ...
                    dest_word = static_cast<unsigned char>(
                        static_cast<uint32_t>(low) | static_cast<uint32_t>(mid) |
        ...
        void copy_with_bit_offset(
            ...
            gsl::span<unsigned char> dest)
        {
            uint32_t dest_next = (dest_bit_offset + 7) >> 3U;
            uint32_t diff = (dest_next * 8U) - dest_bit_offset;
            ...
            if (diff) {
                uint32_t src_begin = src_bit_offset >> 3U;
                uint32_t dest_begin = dest_bit_offset >> 3U;
                uint32_t dest_offset = dest_bit_offset & 7U;
                uint32_t src_offset = src_bit_offset & 7U;
                uint32_t high_diff = src_offset + diff - 8U;
                ...
                if (high_diff <= 0) {
                    ...
                    dest_val = static_cast<unsigned char>(
                        (static_cast<uint32_t>(dest_val) & static_cast<uint32_t>(mask)) |
                        static_cast<uint32_t>(low) | static_cast<uint32_t>(high));
                }
            }
        }
    } // namespace
        ...
        const Modulus &mod)
    {
        ...
        BitstringView<const unsigned char> item_bsw(
            item.value().data(), safe_cast<uint32_t>(item_bit_count));
        ...
        size_t alg_label_padded_size = mul_safe(label_size, felts_per_item);
