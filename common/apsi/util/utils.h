...
namespace apsi::util {
    ...
    */
    std::vector<std::uint64_t> conversion_to_digits(std::uint64_t input, std::uint64_t base);
    ...
    */
    ...
    std::vector<std::pair<T, T>> partition_evenly(T count, T partition_count)
    {
        ...
        partitions.reserve(static_cast<size_t>(partition_count) + 1);
    ...
    */
    // Intentionally also forward-declared in wiping_allocator.h to keep that header lightweight.
    ...
    */
    ...
    seal::parms_id_type get_parms_id_for_chain_idx(
        const seal::SEALContext &seal_context, std::size_t chain_idx);
} // namespace apsi::util
