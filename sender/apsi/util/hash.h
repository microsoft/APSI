...
namespace apsi::sender::util {
    ...
    public:
        ...
        std::uint64_t operator()(uint64_t data) const noexcept
        {
        ...
        std::uint64_t operator()(gsl::span<const std::uint64_t> data) const noexcept
        {
    ...
    private:
        ...
        static constexpr std::size_t block_value_count_ =
            (static_cast<std::size_t>(1) << (8 * block_size_));
