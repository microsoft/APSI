...
shared_ptr<SenderDB> create_sender_db(
    const CSVReader::DBData &db_data, unique_ptr<PSIParams> psi_params, size_t nonce_byte_count);
    ...
    const CSVReader::DBData &db_data, unique_ptr<PSIParams> psi_params, size_t nonce_byte_count)
{
    ...
    if (holds_alternative<CSVReader::UnlabeledData>(db_data)) {
        ...
        try {
            ...
            sender_db =
                make_shared<SenderDB>(*psi_params, label_byte_count, nonce_byte_count, true);
