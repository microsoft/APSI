    ...
    namespace network
    {
        ...
        */
        ...
        public:
            ...
            */
            virtual void send_get_parameters_response(const std::vector<seal::SEAL_BYTE>& client_id, const PSIParams& params) = 0;

            /**
            Send a request to Preprocess items on Sender
            */
            virtual void send_preprocess(const std::vector<seal::SEAL_BYTE>& buffer) = 0;

            /**
            * Send a response to a request to Preprocess items
            */
            virtual void send_preprocess_response(const std::vector<seal::SEAL_BYTE>& client_id, const std::vector<seal::SEAL_BYTE>& buffer) = 0;
            ...
            */
            virtual void send_query_response(const std::vector<seal::SEAL_BYTE>& client_id, const size_t package_count) = 0;

            /**
            * Send a ResultPackage structure
            */
            virtual void send(const std::vector<seal::SEAL_BYTE>& client_id, const ResultPackage& pkg) = 0;
