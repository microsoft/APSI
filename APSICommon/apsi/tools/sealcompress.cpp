...
namespace apsi
{
    ...
    void CiphertextCompressor::compressed_save(const Ciphertext &encrypted, 
        ostream &stream)
    {
        ...
        int compr_coeff_bit_count = parms.plain_modulus().bit_count() + 
            util::get_significant_bit_count(coeff_count);
        ...
        size_t compr_data_uint64_count = util::divide_round_up(compr_data_byte_count, static_cast<size_t>(util::bytes_per_uint64));
        ...
        logging::Log::debug("COMPRESSOR: compressing %i uint64s into %i", encrypted_uint64_count, compr_data_uint64_count);
    ...
    void CiphertextCompressor::compressed_load(istream &stream, 
        Ciphertext &destination)
    {
        ...
        for (size_t i = 0; i < encrypted_uint64_count; i++, destination_coeff_ptr++)
        {
            ...
            *destination_coeff_ptr = shifted_coeff << 
                (util::bits_per_byte * (util::bytes_per_uint64 - compr_coeff_byte_count) - bit_shift);
