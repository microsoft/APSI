    ...
    namespace sender
    {
        ...
        public:
                ...
                :seal_context_(std::move(context)), public_key_(pub_key), evaluation_keys_(eval_keys), local_evaluators_(local_evaluator_num)
            {
                ...
                evaluator_.reset(new seal::Evaluator(*seal_context_, evaluation_keys_));

                for (int i = 0; i < local_evaluator_num; i++)
                    local_evaluators_[i].reset(new seal::Evaluator(*seal_context_, evaluation_keys_, seal::MemoryPoolHandle::acquire_new(false)));
            }
            ...
            {
                for (int i = 0; i < local_evaluator_num; i++)
                    local_evaluators_[i].reset(new seal::Evaluator(*seal_context_, seal::MemoryPoolHandle::acquire_new(false)));
            }
            ...
            void set_evaluation_keys(const seal::EvaluationKeys &evaluation_keys, int local_evaluator_num = 0)
            {
