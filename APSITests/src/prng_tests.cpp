...
namespace APSITests
{
	TEST(PRNGTests, constructor_test)
	{
		block seed;
		PRNG prng1(seed);
		PRNG prng2(seed);
		...
		ASSERT_EQ((size_t)100, buffer1.size());
		ASSERT_EQ((size_t)100, buffer2.size());
		...
		PRNG prng3;
		ASSERT_ANY_THROW(prng3.get<u64>());

		// After seeding it should not throw
		prng3.set_seed(seed);
		ASSERT_NO_THROW(prng3.get<u64>());
	}

	TEST(PRNGTests, get_test)
	{
		...
		prng1.get(buffer1.data(), buffer1.size());
		prng2.get(buffer2.data(), buffer2.size());
		ASSERT_TRUE(0 != memcmp(buffer1.data(), buffer2.data(), buffer1.size()));

		// Using the same seed should yield the same numbers
		vector<u8> buffer3(100);
		PRNG prng3(seed2);
		prng3.get(buffer3.data(), buffer3.size());
		ASSERT_TRUE(0 == memcmp(buffer2.data(), buffer3.data(), buffer2.size()));
	}

	TEST(PRNGTests, get_more_than_buffer)
	{
		...
		ASSERT_EQ((size_t)2000, buffer.size());
		...
		for (auto& blk : buffer)
		{
			...
			{
				// Should not find an existing u64 with same value
				FAIL();
			}
		...
		for (auto& blk : buffer2)
		{
			if (set.find(blk) == set.end())
			{
				// Should have found a u64 with the same value
				FAIL();
			}
		}
	}
