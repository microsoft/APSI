...
namespace APSITests
{
	TEST(ItemTests, parse_test)
	{
		...
		ASSERT_EQ((u64)0xFFFFFFFFFFFFFFFF, item[0]);
		ASSERT_EQ((u64)0xFFFFFFFFFFFFFFFF, item[1]);
		...
		ASSERT_EQ((u64)0x8000000000000000, item[1]);
		ASSERT_EQ((u64)0x0000000000000001, item[0]);

		input = "FEDCBA9876543210";
		item.parse(input, /* base */ 16);

		ASSERT_EQ((u64)0xFEDCBA9876543210, item[0]);
		ASSERT_EQ((u64)0, item[1]);

		input = "abcdef";
		item.parse(input, /* base */ 16);

		ASSERT_EQ((u64)0xABCDEF, item[0]);
		ASSERT_EQ((u64)0, item[1]);
		...
		ASSERT_EQ((u64)12345, item[0]);
		ASSERT_EQ((u64)0, item[1]);

		input = "9223372036854775807";
		item.parse(input, /* base */ 10);

		ASSERT_EQ((u64)0x7FFFFFFFFFFFFFFF, item[0]);
		ASSERT_EQ((u64)0, item[1]);

		input = "2361200000000000000000";
		item.parse(input, /* base */ 10);

		ASSERT_EQ((u64)0x003b89d384580000, item[0]);
		ASSERT_EQ((u64)0x80, item[1]);
	}
	...
	TEST(ItemTests, parse_non_regular_string_test)
	{
		...
		ASSERT_EQ((u64)45321, item[0]);
		ASSERT_EQ((u64)0, item[1]);

		item.parse("800000000000000000000001ABCDG", /* base */ 16);

		ASSERT_EQ((u64)0x1ABCD, item[0]);
		ASSERT_EQ((u64)0x800000000000, item[1]);
	}
