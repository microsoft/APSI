...
namespace APSITests {
    ...
    TEST(BinBundleTests, BinBundleUnlabeledCreate)
    {
        ...
        ASSERT_THROW(BinBundle bb(context, 0, 50, 0, true, false), invalid_argument);

        context.set_evaluator();
        BinBundle bb(context, 0, 50, 0, true, false);
    ...
    TEST(BinBundleTests, BinBundleLabeledCreate)
    {
        auto test_fun = [&](size_t label_size) {
            ...
            // No evaluator set in context
            ASSERT_THROW(BinBundle bb(context, label_size, 50, 0, true, false), invalid_argument);

            context.set_evaluator();
            BinBundle bb(context, label_size, 50, 0, true, false);
    ...
    TEST(BinBundleTests, SaveLoadUnlabeled)
    {
        ...
        BinBundle bb(
            context,
            0,
            get_params()->table_params().max_items_per_bin,
            get_params()->query_params().ps_low_degree,
            true,
            false);
        bb.regen_cache();
        ASSERT_TRUE(bb.empty());
