...
namespace APSITests
{
    TEST(PSIParamsTest, Constructor)
    {
        ...
        // All good parameters
        ASSERT_NO_THROW(PSIParams psi_params(item_params, table_params, seal_params));

        // Too short item (4 * 16 == 64 < 80)
        item_params.felts_per_item = 4;
        ASSERT_THROW(PSIParams psi_params(item_params, table_params, seal_params), invalid_argument);
    ...
    TEST(PSIParamsTest, SaveLoadPSIParams)
    {
        ...
        PSIParams psi_params(item_params, table_params, seal_params);
        stringstream ss;
        ...
        ASSERT_EQ(psi_params.table_params().window_size, load_params.table_params().window_size);
    }
}
